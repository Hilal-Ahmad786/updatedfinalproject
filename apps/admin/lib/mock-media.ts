...
const defaultFiles: MediaFile[] = [
  ...
  },
  {
    ...
    caption: 'Professional headshot for team page',
    uploadedAt: new Date(Date.now() - 345600000).toISOString(),
    updatedAt: new Date(Date.now() - 345600000).toISOString(),
    uploadedBy: 'admin',
    tags: ['profile', 'team', 'headshot'],
    folder: 'uploads'
  }
]
...
export function searchFiles(query: string) {
  ...
  return mediaFiles.filter(file => 
    ...
    file.altText?.toLowerCase().includes(lowercaseQuery) ||
...
export function deleteFolder(id: string): boolean {
  ...
  if (['uploads', 'blog-images', 'documents'].includes(id)) {
    return false
  }
...
export function getMediaStats() {
  const stats = {
    ...
    byType: {
      ...
      documents: mediaFiles.filter(f => f.mimeType.includes('pdf') || f.mimeType.includes('document')).length,
      audio: mediaFiles.filter(f => f.mimeType.startsWith('audio/')).length,
      other: mediaFiles.filter(f => !f.mimeType.startsWith('image/') && !f.mimeType.startsWith('video/') && !f.mimeType.startsWith('audio/') && !f.mimeType.includes('pdf') && !f.mimeType.includes('document')).length
    },
...
export function getFeaturedImages() {
  return mediaFiles.filter(file => 
    ...
    (file.tags.includes('featured') || file.folder === 'blog-images')
  )
}